...
## 2026-09-23 inceleme notu

- Varsayılan evaluation, `configs/profiles/isaac-g1-sonic-blockstacking-dex3.json` profilini kullanıyor. Bu profilin `sonic_mujoco` kütle, armatür ve sürtünme hizalaması, SONIC'in dağıtım MuJoCo modeliyle seçilmiş parametrelerde eşleşiyor. Kontrol yasası da aynı: `τ_ff + Kp(q_target−q) + Kd(dq_target−dq)`. PhysX/USD ile MuJoCo'nun COM, atalet, çarpışma ve çözücü davranışlarının birebir eşdeğerliği kanıtlanmadı.
- Ayrı `sonic_training` profili resmi eğitim rig'inin birebir kopyası değil. Resmi `g1.py` dosyası `data/assets/robot_description/urdf/g1/main.urdf` seçiyor (29 hareketli eklem); profil Dex3 eklenmiş `g1_29dof_with_hand_rev_1_0.urdf` kullanıyor (43 hareketli eklem). Bu profil ancak yaklaşık karşılaştırma için kullanılmalı.
- SONIC'in gönderdiği eklem hedefi ile gerçekleşen poz arasındaki büyük yükseklik farkı için en güçlü doğrulanmış mekanizma yerçekimi altında düşük kazançlı, sıfır feed-forward torklu PD takibi. Aşağıdaki A4 deneylerinin telafi sonucu güçlüdür; fakat telafi policy'nin gördüğü plant'i değiştirir ve bazı duruşları bozmuştur. Varsayılanı değiştirmedik.
- SONIC DDS kullanan Isaac oturumlarında `/World/SonicTargetSkeleton` sahne öğesi varsayılan olarak gizlidir. Isaac Stage panelinde bu öğenin tek göz simgesi `Joints` ve `Bones` alt öğelerini birlikte açar. Hedef geometri geçerli SONIC komutları geldikçe gizliyken de güncellenir; VLA, CLI bayrağı, kısayol veya ek kontrol penceresi gerekmez. Komut geçersizleşince geometri temizlenir, göz simgesi tercihi korunur. Sarı noktalar ve camgöbeği çizgiler görseldir; fiziksel çarpışma veya tork uygulamazlar. Dex3 için 29 gövde + 14 el hedefi ve palm bağlantıları, Inspire için yalnız 29 gövde hedefi çizilir; farklı Inspire el geometrisi Dex3 URDF'siyle gösterilmez. Matematik testleri ve MuJoCo kol bağlantısı karşılaştırması geçti; canlı Isaac koşusunda 43 hedef eklemiyle çizim güncellemesi kaydedildi.
- Başlatıcı varsayılan olarak simülasyon saatini kullanır; GR00T 50 Hz ve Ψ₀ 30 Hz sözleşmesi kod ve önceki canlı ölçümlerle desteklenir. Ψ₀ değerlendirmesinin varsayılan guided RTC modu eğitimdeki `model.rtc=false` ile eşleşmez. Eğitim semantiğini denemek için `--psi0-rtc-off` açıkça seçilir. Bu SONIC değerlendirme yolunda maskeli iki neck kanalı artık varsayılan olarak yok sayılır; `--psi0-neck-policy error` sıkı davranışı geri getirir. Atılan değerlerin tamamını saklamak için isteğe bağlı `--telemetry-dir DIR` kullanılır; atma sayısı oturum durumunda görünür.
