...
# Project-Instinct G1 parkour playback.  Like the isaac-g1 entries this streams
# its UI by default (see g1_mode_args); the playback keeps its own loop, so it
# runs foreground and is stopped with Ctrl+C here or by closing the client.
run_instinct_parkour() {
  ...
  up_once
  DC exec -e DISPLAY="$DISPLAY" "${LIVESTREAM_ENV[@]}" dev bash -lc '
    source /opt/humanoid-lab/entrypoint.sh
    use-isaac-sonic
...
case "${1:-}" in
  ...
  isaac-g1-sonic)
    ...
    run_isaac_g1 "$profile_file" "${G1_ARGS[@]}" "${@:3}"
    ;;
  isaac-g1-sonic-rough)
    # The same SONIC path on the InstinctLab parkour terrain instead of the flat
    # plane: stairs, gaps and obstacle fields, with the robot starting on the
    # easiest tile.  See docs/sonic-isaac.md.
    [ "${2:-}" = "dex3" ] || { echo "usage: $0 isaac-g1-sonic-rough dex3 [--head-camera-window] [--duration SECONDS] [--record-video PATH] [--controller none] [--gui|--headless]" >&2; exit 2; }
    g1_mode_args isaac-g1-sonic-rough "${@:3}"
    up_once
    run_isaac_g1 configs/profiles/isaac-g1-sonic-rough-dex3.json "${G1_ARGS[@]}" "${@:3}"
    ;;
  instinct-parkour)
    ...
    # upstream task script.  The streamed UI shows the robot, with the head
    # camera's depth image beside it.  See docs/instinct-parkour.md.
    g1_mode_args instinct-parkour "${@:2}"
    run_instinct_parkour "${G1_ARGS[@]}" "${@:2}"
    ;;
