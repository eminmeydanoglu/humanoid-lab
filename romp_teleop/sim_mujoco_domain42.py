...
from gear_sonic.utils.mujoco_sim import unitree_sdk2py_bridge as _bridge  # noqa: E402

SIM_DDS_DOMAIN_ID = 42
SIM_DDS_INTERFACE = "lo"

_orig_load_wbc_yaml = _configs.BaseConfig.load_wbc_yaml
_orig_elastic_advance = _bridge.ElasticBand.Advance

DROP_MARKER = os.environ.get("SONIC_SIM_DROP_MARKER", "/tmp/romp_drop_robot")
...
def _elastic_advance_with_auto_drop(self, pose):
    """Mirror MuJoCo's `9` key once deploy has armed the controller."""
    if self.enable and os.path.exists(DROP_MARKER):
        self.enable = False
        try:
            os.unlink(DROP_MARKER)
        except FileNotFoundError:
            pass
        print(f"[sim42] deploy armed; elastic band disabled via {DROP_MARKER}", flush=True)
    return _orig_elastic_advance(self, pose)


_bridge.ElasticBand.Advance = _elastic_advance_with_auto_drop
