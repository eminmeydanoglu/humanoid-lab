...
Output modes:
  --control-mode full        SONIC v3 / SMPL encoder mode 2 (default)
  --control-mode upper-body  SONIC v1 / teleop encoder mode 1; fixed standing
                             legs and ROMP wrist/torso targets
"""
...
def parse_args():
    ...
    p.add_argument("--control-mode", choices=["full", "upper-body"], default="full",
                   help="full SMPL tracking, or standing legs + wrist/torso targets")
    p.add_argument("--fps", type=float, default=50.0, help="Publish rate (SONIC 50 Hz)")
    p.add_argument("--loop", action="store_true")
    p.add_argument("--smooth", type=float, default=0.75,
                   help="EMA history weight (0=off, 0.6-0.85 smoother); GEM default 0.75")
    p.add_argument("--root-mode", choices=["full", "yaw", "identity"], default="full",
                   help="full = GEM-compatible gravity-aligned root (use for teleop); "
                        "yaw/identity are diagnostics only")
...
def main():
    ...
    if args.root_mode != "full":
        print("[stage2] WARNING: root-mode=%s is a diagnostic mode; it removes "
              "body pitch/roll from the GEM-compatible teleop stream" % args.root_mode)

    from romp_sonic.coordinates import RompToSmpl24
    from romp_sonic.protocol import PlannerUpperBodyPublisher, V3Publisher
    ...
    publisher_cls = PlannerUpperBodyPublisher if args.control_mode == "upper-body" else V3Publisher
    publisher = None if args.dry_run else publisher_cls(
        port=args.port, topic="pose", sonic_root=args.sonic_root)

    def upper_body_targets(joints, body_quat):
        # SMPL-24 indices: 20 left wrist, 21 right wrist, 15 head.  SONIC's
        # third VR point controls torso_link with a +0.35 m head proxy offset.
        position = np.concatenate((joints[20], joints[21], joints[15])).astype(np.float32)
        # ROMP currently does not export reliable per-wrist orientations.  Root
        # orientation preserves torso pitch/roll/heading; neutral wrist quats
        # avoid injecting fabricated wrist rotations into the policy.
        identity = np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float32)
        orientation = np.concatenate((identity, identity, body_quat)).astype(np.float32)
        ...
        if args.dry_run:
            for i in range(len(j_out)):
                if i % max(1, len(j_out) // 10) == 0:
                    vr_pos, _ = upper_body_targets(j_out[i], q_out[i])
                    print(f"[dry-run] mode={args.control_mode} i={i} smpl_joints{j_out[i].shape} "
                          f"range=[{j_out[i].min():.3f},{j_out[i].max():.3f}] "
                          f"quat_norm={np.linalg.norm(q_out[i]):.4f} "
                          f"vr_range=[{vr_pos.min():.3f},{vr_pos.max():.3f}]")
            return len(j_out)

        sent = 0
        t_start = time.time()
        for i in range(len(j_out)):
            if args.control_mode == "upper-body":
                vr_pos, vr_orn = upper_body_targets(j_out[i], q_out[i])
                idx = publisher.publish(vr_pos, vr_orn, q_out[i])
            else:
    ...
    try:
        if args.play:
            ...
            while True:
                ...
                j, q = smoother(j, q)
                if args.control_mode == "upper-body":
                    vr_pos, vr_orn = upper_body_targets(j, q)
                    publisher.publish(vr_pos, vr_orn, q)
                else:
