...
class NativeReturnIntegrationTests(unittest.TestCase):
    ...
    def test_body_result_is_received_when_its_frame_precedes_next_outbound_action(self):
        ...
        time.sleep(0.15)
        publisher.send(BodyCommand(7, 123, (0.25,) * 29, (0.5,) * 29).pack())
    ...
    def test_native_source_has_independent_state_drain_and_v4_frame_sequence(self):
        ...
        self.assertIn("kAngularOffset = kToken", native)
        self.assertIn("kGravityOffset", native)
        self.assertIn("state + 61", native)
        self.assertIn("std::memcpy(input.data(), fields[0].data", native)
        self.assertIn("read_i64(fields[1].data)", native)
        self.assertIn("native_body_publish", native)
        self.assertIn("kIsaacLabToMujoco", native)
        self.assertIn("kDefaultAngles", native)
        self.assertIn("kActionScale", native)
        self.assertIn("const auto raw_action = decoder.run(input)", native)
        self.assertIn("action[kIsaacLabToMujoco[i]] * kActionScale[i]", native)
        self.assertIn("last_command_sequence", isaac)
        self.assertIn("isaac_body_receive", isaac)
        self.assertIn("native_decoder_state", isaac)
        self.assertIn("inference_worker", isaac)
        self.assertIn("latency_compensated_chunk_index", isaac)
        self.assertIn("external_groot_encoder_bypass", native)
