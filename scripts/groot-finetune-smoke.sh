...
materialize_checkpoint_processor() {
  local checkpoint="$1"
  [[ -d "$checkpoint/processor" ]] && return 0

  # The pinned N1.7 launcher writes these files at the checkpoint root.
  require_file "$checkpoint/processor_config.json" "flattened checkpoint-2 processor config"
  require_file "$checkpoint/statistics.json" "flattened checkpoint-2 processor statistics"
  mkdir -p "$checkpoint/processor"
  cp -p "$checkpoint/processor_config.json" "$checkpoint/statistics.json" "$checkpoint/processor/"
  if [[ -f "$checkpoint/embodiment_id.json" ]]; then
    cp -p "$checkpoint/embodiment_id.json" "$checkpoint/processor/"
  fi
}
...
validate_training_artifacts() {
  ...
  materialize_checkpoint_processor "$checkpoint"
